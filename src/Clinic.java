...
class Clinic {
    ...
    public void getSchedule() {
    	...
    	int noOfPatientInOneDay=0;
    	Iterator<Patient> i= patientQueue.iterator();
    	while(i.hasNext() && noOfPatientInOneDay<3) {
    		Patient q=i.next();
    			q.setAppointmentDay("Day "+dayCount);
    			noOfPatientInOneDay++;
    			admittedPatients.add(q);
    			System.out.println("Sceduled "+q.getName()+" on "+q.getAppointmentDay());
    			i.remove();
    	}
    	 dayCount=dayCount+1;
        // TODO
    ...
    public void getAppointmentDetails(Patient patient) {
        ...
        // Else print "Patient not found in the admitted list."
        if(admittedPatients.contains(patient)) {
         	System.out.println("Appointment Details: "+patient.getName()+" schedule on  "+patient.getAppointmentDay());
         }
         else {
         	System.out.println("Patient not found in the admitted list.");
         }
